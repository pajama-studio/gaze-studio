...
An open source workspace for aligned video and gaze replay, dynamic areas of interest, and reproducible eye tracking analysis. English interface. Browser analysis, Cloudflare infrastructure.
...
![Gaze Studio replaying a real GazeMining recording with aligned gaze and areas of interest](docs/screenshots/replay.png)
...
- Run I-VT / I-DT fixation detection, tracking-quality checks, AOI dwell / visits / TTFF / transitions, pupil summaries, participant filtering, time windows, and CSV/JSON exports in a browser worker.
...
The default example is a **real 78-second GazeMining recording**. A separate synthetic fixture tests the software; it is explicitly labelled and is never evidence of model accuracy. See [data provenance](docs/gazemining-provenance.json).

## Run locally

Node 22.22+ recommended.
...
Workers serves the app and API; D1 stores ownership and metadata; R2 stores media, gaze, raw attachments and analysis snapshots; Workers AI supplies optional object detection. Browser workers perform interactive numerical analysis. No external application server is required.
...
Gaze Package 0.1 is an experimental application interchange format, **not an established industry standard**. BIDS 1.11 already defines eye tracking physiological files. We preserve interoperability while adding the explicit media clock, frame index, portable assets, AOI keyframes and checksums a web player needs.
...
New application and analysis code: [MIT](LICENSE). The GazeMining sample remains under its original CC0 dedication; displayed third-party page content retains its own rights. Fonts use SIL OFL. See [THIRD_PARTY.md](THIRD_PARTY.md). Existing private Cogix repositories were used to understand interfaces; their code was not copied into this public project.
