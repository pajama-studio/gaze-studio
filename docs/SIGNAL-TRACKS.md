...
A same-host Chromium check with 4× CPU throttling and five seconds of playback compared the preceding editor with the five-track working build: main-thread script work was 312.9 ms vs 375.6 ms, task time 1,204.2 ms vs 1,400.0 ms, and rAF P95 19.9 ms vs 20.2 ms. Neither run observed a task over 50 ms. The new tracks add work; this single pair does not establish equal performance on every device. [Recorded measurement](verification/v0.2.1-performance.json).
