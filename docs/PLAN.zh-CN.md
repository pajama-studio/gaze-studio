...
## 2026-09-06 v0.2 实现进展：双眼图像、Rust 和云端数据

已按“原始双眼=左右眼相机图像”实现三路视频同步回放。默认真实样本为 Through the Eyes of Emotion P01/0a 的 20 秒片段，包含独立左右眼 IR 图像、VR 场景录像、gaze、每眼瞳孔信号；原始完整试次与派生文件分别存入自己的 Cloudflare R2。HARMONIC 官方完整样本及其 NPY 时间戳也已归档，并有可复现转换器。HARMONIC 明确的每帧采集时间戳用于解决不同编码时长与丢帧；公开 Emotion 演示的 gaze/scene 对齐为估计，不能用残差冒充 tracker 精度。

核心 I-VT/I-DT、AOI、质量计算迁入 Rust，编译为 WASM，在真实 Web Worker 池中运行。按完整 participant 流并行，避免切断 fixation 边界；单人试次使用一个计算 worker，多个试次也可共用线程池。瞳孔的每眼均值、分位数、标准差与有效时间覆盖同样由 Rust 计算。未声称单人数据内部采用 Rayon 或获得未经测量的加速倍数。

代码拆为 `gaze-core`、`gaze-analysis`、`gaze-react` 三个独立 workspace 包，回放、双眼视频、时间轴、热图、瞳孔曲线可以分别复用；`/embed/` 是脱离 Studio 的实际集成例子。后端 AOI 为真实 Workers AI DETR，当前帧和采样区间均可处理。SAM 尚未部署；下一阶段应在明确模型和权重许可后实测 CPU Container 推理，支持 mask 到 polygon 的版本化转换。

Cloudflare 结构：R2 保存原始/派生/分析产物，D1 `datasets` + `dataset_assets` 保存来源、许可、版本、SHA-256、公开范围。目录先上传文件后更新索引。私人上传继续使用独立 session workspace；curated 数据集由部署工具维护。当前已实现的是公开真实样本和项目实际使用的数据归档，不等于全量复制所有研究目录中的数据集。

后续顺序：① 对齐协议加入更完整的每帧丢失区间与物理时延验证；② 单试次流式 chunk 内核、Arrow/Parquet 查询与 Workflows/Queues；③ SAM/实例分割与跨帧对象身份跟踪、人工复核版本；④ pursuit、角度单位眼跳、眨眼事件和瞳孔预处理的独立有效性验证；⑤ 团队权限、多被试统计、完整 BIDS 导出与 webcam 模型评估。当前已有的描述统计不输出认知负荷、情绪或临床诊断。

实现、格式与数据来源详见 [模块说明](MODULES.md)、[原始双眼数据](BINOCULAR.md)、[Cloudflare 数据目录](CLOUDFLARE-DATA.md)。


编辑器交互已升级：石墨灰工作台、场景监视器、左右并排眼部监视器、每眼信号与底部统一多轨时间轴；支持拖动、缩放、键盘逐帧、旋转视图、折叠眼部面板和 AOI 撤销/重做。高频播放时钟从 App 状态拆为外部 store，避免侧栏/检查器随每帧重渲染；静态时间轴片段与参与者索引缓存，拖动事件用 rAF 合并，导入/benchmark 页面按需加载。测试记录包含 4 倍 CPU 限速下的本机前后对照；不把单次测量泛化为所有设备的性能承诺。

## 2026-09-06：真实样例与数据轨道补齐

已从工作台及生产静态资源移除 Attention Garden，合成数据只用于测试。Emotion 左眼默认显示旋转 180°，右眼为 0°，保留原始朝向查看和可保存旋转；不修改原始视频或 gaze 坐标。

新增可复用 SignalTimeline：gaze X/Y、注视、扫视候选、双眼瞳孔与质量五类轨道，与原有媒体/AOI 共用时间轴、缩放和游标。事件可查看指标、定位回放和导出 CSV。扫视候选由 Rust 推导，采用明确的速度阈值、10 ms 最短持续时间及近重合时间戳排除，尚不是验证过的生理分类器。低置信度和无效 gaze 不会被冒充为眨眼；只有源数据的 blink 标记显示为已记录眨眼。当前 Emotion 只有一套场景 gaze，不伪造左右眼独立 gaze。详见 [SIGNAL-TRACKS.md](SIGNAL-TRACKS.md)。
